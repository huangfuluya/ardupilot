...
void AP_CANManager::init()
{
    ...
    for (uint8_t i = 0; i < HAL_NUM_CAN_IFACES; i++) {
        ...
        case AP_CAN::Protocol::ZYServo:
            _drivers[drv_num] = _drv_param[drv_num]._zyservo = NEW_NOTHROW AP_ZYServo;
            if (_drivers[drv_num] == nullptr) {
                AP_BoardConfig::allocation_error("ZYServo %d", drv_num + 1);
                continue;
            }
            AP_Param::load_object_from_eeprom((AP_ZYServo*)_drivers[drv_num], AP_ZYServo::var_info);
            break;
#endif
        default:
            continue;
