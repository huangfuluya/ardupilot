...
public:
	...
	virtual int WritePosEx(uint8_t ID, int16_t Position, uint16_t Time, uint16_t Speed);//普通写单个舵机位置指令
	virtual int RegWritePosEx(uint8_t ID, int16_t Position, uint16_t Speed, uint8_t ACC = 0);//异步写单个舵机位置指令(RegWriteAction生效)
	virtual void SyncWritePosEx(uint8_t ID[], uint8_t IDN, int16_t Position[], uint16_t Speed[], uint8_t ACC[]);//同步写多个舵机位置指令
