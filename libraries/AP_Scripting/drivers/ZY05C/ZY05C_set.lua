...
local SERVO_ID_LIST = {0x10} -- 0~63,即：0x00~0x3F，从0x10开始
local GCS_ID_LIST = {0x3F} -- 0~63,即：0x00~0x3F, 从0x3F开始
local SERVO_FUNCTION_LIST = {141}
local SERVO_ANGLE_MAX = {120}  -- 伺服最大角度，单位度，最大120度
...
function set_id(servo_id, gcs_id)
    msg = CANFrame()
...
function read_id()
    ...
    -- Extended frame, node ID 0x143F4401
    msg:id(make_ext_id_hilo(0x143F, 0x4401))
    msg:data(0, 0x02) --0x01:设定设备ID值 0x02：读取产品当前ID值
    msg:dlc(8)

    driver:write_frame(msg, uint32_t(10000))
end

function send_check_cmd(gcs_id)
    msg = CANFrame()
    msg:id(make_ext_id_hilo(0x1400 | gcs_id, 0x4402))
    msg:dlc(8)
    driver:write_frame(msg, uint32_t(10000))
end
...
function send_angle(gcs_id, angle)
    msg = CANFrame()
    msg:id(make_ext_id_hilo(0x1400 | gcs_id, 0x4405))
    -- 先传低字节，再传高字节
...
function receive_set_ack()
    ...
    if eff_id == uint32_t(0x1411FC01) then
        ...
        if frame:data(0) == 0x01 then
            ...
            return true
        elseif frame:data(0) == 0x02 then
            gcs:send_text(0, "Set ID failed")
            return false
        elseif frame:data(0) == 0x00 then
            gcs:send_text(0, string.format("Servo ID:0X%X", frame:data(2)))
            gcs:send_text(0, string.format("GCS ID:0X%X", frame:data(4)))
            return true
        else
            gcs:send_text(0, string.format("0X%X: Unknown ack", frame:data(0)))
            return false
        end
    end
    return false
...
function receive_check_ack(servo_id)
    ...
    -- 打印expected和eff_id用于调试
    -- gcs:send_text(0, string.format("expected=%s eff_id=%s", u32_hex(expected), u32_hex(eff_id))) 
...
function init(servo_id, gcs_id)
    local flag_set = false
    local flag_read = false
    local flag_check = false
    local time_stamps = millis()

    set_id(servo_id, gcs_id)
    ...
    read_id(servo_id)
    time_stamps = millis()
    while not flag_read and millis() - time_stamps < 1000 do
        -- wait for the ack
        flag_read = receive_set_ack()
    end

    -- send_check_cmd(gcs_id)
    -- time_stamps = millis()
    ...
    if flag_set and flag_read then
        init_done = true
        gcs:send_text(0, "Init done")
    else
        gcs:send_text(0, "Init failed")
    end
end
...
function update()
    ...
    -- don't init the servo at the real flight, unless you want to change the ID
        init(SERVO_ID_LIST[1], GCS_ID_LIST[1])
        return update, 10000
    end
    -- 读取第一个通道的输出值
