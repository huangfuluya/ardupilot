...
@ingroup setup
*/
void ModbusMaster::begin(uint8_t slave, AP_HAL::UARTDriver &serial)
{
...
void ModbusMaster::sendBit(bool data)
{
  ...
  if ((u16TransmitBufferLength >> 4) < ku8MaxBufferSize)
  {
    ...
    BIT_WRITE(_u16TransmitBuffer[_u8TransmitBufferIndex], txBitIndex, data);
...
*/
void ModbusMaster::idle(void (*idle_fun)())
{
  _idle = idle_fun;
}
...
*/
void ModbusMaster::preTransmission(void (*preTransmission_fun)())
{
  _preTransmission = preTransmission_fun;
}
...
*/
void ModbusMaster::postTransmission(void (*postTransmission_fun)())
{
  _postTransmission = postTransmission_fun;
}
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  while (u8BytesLeft && !u8MBStatus)
  {
    ...
    if ((AP_HAL::millis() - u32StartTime) > ku16MBResponseTimeout)
    {
    ...
    switch(u8ModbusADU[1])
    {
      ...
      case ku8MBReadDiscreteInputs:
        ...
        for (i = 0; i < (u8ModbusADU[2] >> 1); i++)
        {
          ...
          {
            _u16ResponseBuffer[i] = UINT16_VALUE(u8ModbusADU[2 * i + 4], u8ModbusADU[2 * i + 3]);
          }
        ...
        if (u8ModbusADU[2] % 2)
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = UINT16_VALUE(0, u8ModbusADU[2 * i + 3]);
          }
      ...
      case ku8MBReadWriteMultipleRegisters:
        ...
        for (i = 0; i < (u8ModbusADU[2] >> 1); i++)
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = UINT16_VALUE(u8ModbusADU[2 * i + 3], u8ModbusADU[2 * i + 4]);
          }
